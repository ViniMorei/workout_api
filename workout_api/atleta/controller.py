...
async def query(
    ...
    atletas = [AtletaOut.model_validate(atleta) for atleta in result]

    for atleta in atletas:
        atleta.categoria = (await db_session.execute(
            select(CategoriaModel).filter_by(pk_id=atleta.categoria_id))
                            ).scalars().first()
        atleta.centro_treinamento = (await db_session.execute(
...
# Adicionando o router de atletas
